...
public class LoginUserArgumentResolver implements HandlerMethodArgumentResolver {
    ...
    public boolean supportsParameter(MethodParameter parameter) {
        ...
        // 파라미터에 대해 이 해결사를 동원할 수 있는지 여부를 알려줌
        // 즉, 다음 두 가지 조건을 만족해야만 이 해결사가 파라미터 값을 줄 수 있다.

        // 1) 해당 파라미터에 @LoginUser 애노테이션이 붙어있어야 한다.
        // 2) 해당 파라미터는 User 객체를 받을 수 있는 타입이어야 한다.
        return parameter.hasParameterAnnotation(LoginUser.class) &&
                parameter.getParameterType().isAssignableFrom(User.class); // 파라미터 타입이 User 객체를 담을 수 있는가
    }
    ...
    public Object resolveArgument(MethodParameter parameter, ModelAndViewContainer mavContainer, NativeWebRequest webRequest, WebDataBinderFactory binderFactory) throws Exception {
        ...
        // 파라미터 값을 리턴함
        // 로그인 사용자 정보(User 객체를 세션에서 꺼내 리턴함)
