...
public class ProjectDaoImpl implements ProjectDao {
    ...
    public List<Project> list() throws Exception {
        return sqlSession.selectList("select project_id as no, title, start_date as startDate, end_date as endDate from myapp_projects order by project_id asc", Project.class);
    }

    @Override
    public Project findBy(int no) throws Exception {
        return sqlSession.selectOne("select project_id as no, title, description, start_date as startDate, end_date as endDate from myapp_projects where project_id=?", Project.class, no);
    }
    ...
    public boolean delete(int no) throws Exception {
        int count = sqlSession.delete("delete from myapp_projects where project_id=?", no);
    ...
    public boolean insertMembers(int projectNo, List<User> members) throws Exception {
        for (User user : members) {
            sqlSession.insert("insert into myapp_project_members(project_id, user_id) values (?, ?)", projectNo, user.getNo());
        }
        return true;
    }
    ...
    public List<User> getMembers(int projectNo) throws Exception {
        return sqlSession.selectList("select "
                ...
                + " where pm.project_id=?", User.class, projectNo);
    }

    @Override
    public boolean deleteMembers(int projectNo) throws Exception {
        return sqlSession.delete("delete from myapp_project_members where project_id=?", projectNo) > 0;
    }
}
