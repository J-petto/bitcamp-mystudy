...
public class SqlSession {
    ...
    private void setPropertyValue(Object obj, ResultSet rs, String columnName)
        ...
        } else { // ex_ writer_name
            Method getter = findMethod(obj.getClass(), toGetterName(names[0]));
            Object embeddedObject = getter.invoke(obj);
            if(embeddedObject == null){
                embeddedObject = getter.getReturnType().getConstructor().newInstance();
                Method setter = findMethod(obj.getClass(), toSetterName(names[0]));
                setter.invoke(obj, embeddedObject);
            }
            callSetter(embeddedObject, names[1], rs, columnName);
        }
    }
    ...
    private void callSetter(Object obj, String propertyName, ResultSet rs, String columnName) throws Exception {
